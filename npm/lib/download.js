...
function assetName(version, label) {
  return `fin-v${version}-${label}.tar.gz`;
}
...
function fetchBuffer(url, redirects = 0) {
  return new Promise((resolve, reject) => {
    ...
    const req = https.get(
      url,
      { headers: { 'User-Agent': 'fin-npm-installer' } },
      (res) => {
...
// Extract the `fin` member from a .tar.gz buffer and return the path to the
// extracted binary inside a temp dir. Relies on the system `tar`, which is
// present on all supported (Unix) targets.
function extractBinary(tarball) {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'fin-'));
  const tarPath = path.join(tmpDir, 'fin.tar.gz');
  ...
  const extracted = path.join(tmpDir, 'fin');
  if (!fs.existsSync(extracted)) {
    throw new Error(`archive did not contain a fin binary`);
  }
  return { extracted, tmpDir };
}

// Download, verify, and install the binary for this platform if not already
// present. Idempotent. Set FIN_SKIP_DOWNLOAD=1 to no-op (e.g. in CI).
async function ensureBinary({ force = false, version = VERSION } = {}) {
  ...
  if (!label) {
    throw new Error(
      `fin has no prebuilt binary for your platform (${key}).\n` +
  ...
  process.stderr.write(`fin: downloading ${assetName(version, label)} …\n`);
  ...
  process.stderr.write(`fin: installed ${label} binary\n`);
  return dest;
}
